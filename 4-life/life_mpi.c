...
void life_init(const char *path, life_t *l) {
    ...
    decomposition(l->nx, l->size, l->rank, &(l->start), &(l->stop));

    int s1, s2;
    decomposition(l->nx, l->size, 0, &s1, &s2);
...
void life_collect(life_t *l) {
    if (l->rank == l->size - 1) {
        ...
        for (i = 0; i < l->size - 1; i++) {
            int s1, s2;
            decomposition(l->nx, l->size, i, &s1, &s2);
            MPI_Recv(l->u0 + ind(s1, 0), 1, l->block_type, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    } else {
        MPI_Send(l->u0 + ind(l->start, 0), 1, l->block_type, l->size - 1, 0, MPI_COMM_WORLD);
    }
}
...
void life_exchange(life_t *l) {
    if (l->size != 1) {
        ...
        MPI_Send(l->u0 + ind(l->stop - 1, 0), 1, l->column_type, right, 0, MPI_COMM_WORLD);
        MPI_Recv(l->u0 + ind(l->start - 1, 0), 1, l->column_type, left, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        
        MPI_Send(l->u0 + ind(l->start, 0), 1, l->column_type, left, 0, MPI_COMM_WORLD);
        MPI_Recv(l->u0 + ind(l->stop, 0), 1, l->column_type, right, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
}